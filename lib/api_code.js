...
exports.queryQuota = async function () {
  ...
  const url = this.wxappPrefix + 'queryquota?access_token=' + componentAccessToken.componentAccessToken
...
exports.speedUpAudit = async function (auditId) {
  const componentAccessToken = await this.ensureComponentToken()
  const url = this.wxappPrefix + 'speedupaudit?access_token=' + componentAccessToken.componentAccessToken
  const data = {
    auditid: auditId
  }
