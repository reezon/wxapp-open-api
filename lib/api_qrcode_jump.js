...
exports.deconsteQrcodeJump = async function (prefix) {
  const token = await this.ensureAccessToken()
  const url = this.wxopenPrefix + 'qrcodejumpdelete?access_token=' + token.accessToken
  const data = {
    prefix: prefix
  }
 ...
 * @param {String} prefix 二维码规则
 */
exports.deleteQrcodeJump = async function (prefix) {
  const token = await this.ensureAccessToken()
  const url = this.wxopenPrefix + 'qrcodejumpdelete?access_token=' + token.accessToken
  const data = {
    prefix: prefix
...
/**
 * 将二维码长链接转成短链接
 * 详情请见：<https://developers.weixin.qq.com/doc/oplatform/Third-party_Platforms/Mini_Programs/qrcode/shorturl.html>
 * @param {String} longUrl
 */
exports.shortUrl = async function (longUrl) {
  const token = await this.ensureAccessToken()
  const url = this.prefix + 'shorturl?access_token=' + token.accessToken
  const data = {
    long_url: longUrl
  }
